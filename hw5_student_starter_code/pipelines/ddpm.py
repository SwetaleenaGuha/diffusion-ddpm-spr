    ...
    def __call__(
        ...
        for t in self.progress_bar(self.scheduler.timesteps):
            ...
            if classes is not None and guidance_scale is not None and guidance_scale != 1.0:
                ...
                t_input = torch.cat([t.unsqueeze(0).repeat(batch_size), t.unsqueeze(0).repeat(batch_size)], dim=0)
            else:
                model_input = image
                t_input = t.unsqueeze(0).repeat(batch_size)
                # NOTE: leave c as None if you are not using CFG
                c = None
            
            # TODO: 1. predict noise model_output
            model_output = self.unet(model_input, t_input, c)
